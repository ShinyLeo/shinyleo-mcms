...
import com.shinyleo.util.RegexUtil;
...
public abstract class IParser {
    ...
    public Map<String, String> getProperty(String regex) {
        HashMap listPropertyMap = new HashMap();
        String listProperty = parseFirst(this.htmlCotent, regex, 1);
        if(listProperty == null) {
            return listPropertyMap;
        } else {
            List listPropertyName = parseAll(listProperty, "(\\w*)\\s*=", 1);
            List listPropertyValue = parseAll(listProperty, "=\\s*(\\w*)", 1);
    ...
    public String replaceAll(String content, String regex) {
        return RegexUtil.replaceAll(this.htmlCotent, regex, content);
    }
